// ESP + Aimbot Base - Offline Roblox Dev Tool
// Copyright (c) 2025 - Open Source for Educational Use
// ONLY FOR YOUR OWN GAMES IN ROBLOX STUDIO

#include <windows.h>
#include <tlhelp32.h>
#include <thread>
#include <chrono>
#include <mutex>
#include <atomic>
#include <vector>
#include <string>
#include <iostream>
#include <fstream>
#include <cmath>
#include <gdiplus.h>
#pragma comment(lib, "gdi32.lib")
#pragma comment(lib, "user32.lib")
#pragma comment(lib, "kernel32.lib")
#pragma comment(lib, "gdiplus.lib")

using namespace Gdiplus;

// UPDATE THESE IF ROBLOX PATCHES (use Cheat Engine)
namespace offsets {
    constexpr uintptr_t Parent = 0x50;
    constexpr uintptr_t Children = 0x80;
    constexpr uintptr_t Name = 0x78;
    constexpr uintptr_t LocalPlayer = 0x130;
    constexpr uintptr_t Character = 0x228;
    constexpr uintptr_t Health = 0x1A0;
    constexpr uintptr_t Team = 0x218;
    constexpr uintptr_t Position = 0x140;
    constexpr uintptr_t DisplayName = 0x118;
}

struct Vector3 { float x, y, z; };
struct Vector2 { float x, y; };
struct Matrix4 { float m[16]; };
struct PlayerInfo {
    Vector3 head, feet;
    float health;
    int team;
};

HANDLE hProcess = nullptr;
HWND hRoblox = nullptr, hOverlay = nullptr;
uintptr_t dataModel = 0;
Matrix4 viewMatrix;
RECT rect;
std::vector<PlayerInfo> players;
std::mutex mtx;

std::atomic<bool> enabled = true, espOn = true, aimOn = false;
float fov = 90.0f, smooth = 0.5f;
Vector3 localHead = {};
int localTeam = 0;

// Read memory
template<typename T> T Read(uintptr_t addr) {
    T v; ReadProcessMemory(hProcess, (LPCVOID)addr, &v, sizeof(T), nullptr); return v;
}
std::string ReadStr(uintptr_t addr) {
    int len = Read<int>(addr);
    char* buf = new char[len + 1];
    ReadProcessMemory(hProcess, (LPCVOID)(addr + 0x8), buf, len, nullptr);
    buf[len] = 0;
    std::string s(buf); delete[] buf;
    return s;
}

// Find child by name
uintptr_t FindChild(uintptr_t parent, const std::string& name) {
    uintptr_t start = Read<uintptr_t>(parent + offsets::Children);
    uintptr_t end = Read<uintptr_t>(start + 0x8);
    for (uintptr_t p = start; p < end; p += 8) {
        uintptr_t child = Read<uintptr_t>(p);
        if (!child) continue;
        if (ReadStr(child + offsets::Name) == name) return child;
    }
    return 0;
}

// Auto-find DataModel from log
std::string GetLatestLog() {
    char path[MAX_PATH];
    SHGetFolderPathA(NULL, CSIDL_LOCALAPPDATA, NULL, 0, path);
    std::string dir = std::string(path) + "\\Roblox\\logs\\";
    WIN32_FIND_DATA fd; HANDLE h = FindFirstFile((dir + "*").c_str(), &fd);
    std::string latest; FILETIME last = {0};
    do {
        if (!(fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
            if (CompareFileTime(&fd.ftLastWriteTime, &last) > 0) {
                last = fd.ftLastWriteTime;
                latest = dir + fd.cFileName;
            }
        }
    } while (FindNextFile(h, &fd));
    FindClose(h);
    return latest;
}
uintptr_t GetDataModel() {
    std::string file = GetLatestLog();
    if (file.empty()) return 0;
    std::ifstream f(file);
    std::string line;
    while (std::getline(f, line)) {
        if (line.find("DataModel") != std::string::npos && line.find("0x") != std::string::npos) {
            uintptr_t addr; sscanf(line.c_str() + line.find("0x"), "0x%llx", &addr);
            return addr;
        }
    }
    return 0;
}

// World to screen
bool W2S(const Vector3& pos, Vector2& out) {
    float w = pos.x * viewMatrix.m[3] + pos.y * viewMatrix.m[7] + pos.z * viewMatrix.m[11] + viewMatrix.m[15];
    if (w < 0.1f) return false;
    float x = pos.x * viewMatrix.m[0] + pos.y * viewMatrix.m[4] + pos.z * viewMatrix.m[8] + viewMatrix.m[12];
    float y = pos.x * viewMatrix.m[1] + pos.y * viewMatrix.m[5] + pos.z * viewMatrix.m[9] + viewMatrix.m[13];
    out.x = (rect.right / 2.0f) * (1 + x / w);
    out.y = (rect.bottom / 2.0f) * (1 - y / w);
    return true;
}

// Find Roblox
bool FindRoblox() {
    HANDLE snap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
    PROCESSENTRY32 pe{ sizeof(pe) };
    if (Process32First(snap, &pe)) {
        do {
            if (strcmp(pe.szExeFile, "RobloxPlayerBeta.exe") == 0) {
                hProcess = OpenProcess(PROCESS_VM_READ | PROCESS_VM_OPERATION, FALSE, pe.th32ProcessID);
                hRoblox = FindWindow(NULL, "Roblox");
                CloseHandle(snap);
                return hProcess && hRoblox;
            }
        } while (Process32Next(snap, &pe));
    }
    CloseHandle(snap);
    return false;
}

// Memory loop
void MemoryLoop() {
    while (enabled) {
        std::this_thread::sleep_for(std::chrono::milliseconds(16));
        dataModel = GetDataModel();
        if (!dataModel) continue;

        uintptr_t players = FindChild(dataModel, "Players");
        if (!players) continue;

        uintptr_t local = Read<uintptr_t>(players + offsets::LocalPlayer);
        if (!local) continue;

        localTeam = Read<int>(local + offsets::Team);
        uintptr_t chara = Read<uintptr_t>(local + offsets::Character);
        if (chara) {
            uintptr_t head = FindChild(chara, "Head");
            if (head) localHead = Read<Vector3>(head + offsets::Position);
        }

        std::vector<PlayerInfo> list;
        uintptr_t start = Read<uintptr_t>(players + offsets::Children);
        uintptr_t end = Read<uintptr_t>(start + 0x8);
        for (uintptr_t p = start; p < end; p += 8) {
            uintptr_t pl = Read<uintptr_t>(p);
            if (pl == local || !pl) continue;
            int team = Read<int>(pl + offsets::Team);
            uintptr_t pc = Read<uintptr_t>(pl + offsets::Character);
            if (!pc) continue;
            uintptr_t hum = FindChild(pc, "Humanoid");
            if (!hum || Read<float>(hum + offsets::Health) <= 0) continue;
            uintptr_t h = FindChild(pc, "Head");
            uintptr_t r = FindChild(pc, "HumanoidRootPart");
            if (!h || !r) continue;
            list.push_back({Read<Vector3>(h + offsets::Position), Read<Vector3>(r + offsets::Position), Read<float>(hum + offsets::Health), team});
        }
        { std::lock_guard<std::mutex> lock(mtx); players = list; }
    }
}

// Hotkeys
void HotkeyLoop() {
    while (true) {
        std::this_thread::sleep_for(std::chrono::milliseconds(50));
        if (GetAsyncKeyState(VK_INSERT) & 1) enabled = !enabled;
        if (GetAsyncKeyState(VK_HOME) & 1) espOn = !espOn;
        if (GetAsyncKeyState(VK_END) & 1) aimOn = !aimOn;

        system("cls");
        std::cout << "Roblox Offline Dev Tool\n";
        std::cout << "Status: " << (enabled ? "ON" : "OFF")
                  << " | ESP: " << (espOn ? "ON" : "OFF")
                  << " | Aimbot: " << (aimOn ? "ON" : "OFF") << "\n";
        std::cout << "Insert=Toggle | Home=ESP | End=Aimbot (Hold LMB)\n";
    }
}

// Aimbot
void AimbotLoop() {
    while (true) {
        std::this_thread::sleep_for(std::chrono::milliseconds(16));
        if (!aimOn || !(GetAsyncKeyState(VK_LBUTTON) & 0x8000)) continue;

        Vector3 target; float best = 99999;
        { std::lock_guard<std::mutex> lock(mtx);
            for (const auto& p : players) {
                float dist = sqrt(pow(p.head.x - localHead.x, 2) + pow(p.head.z - localHead.z, 2));
                if (dist < best && dist < fov) { best = dist; target = p.head; }
            }
        }
        if (best > fov) continue;

        Vector2 scr; if (!W2S(target, scr)) continue;
        POINT mouse; GetCursorPos(&mouse);
        int dx = (scr.x - mouse.x) * smooth;
        int dy = (scr.y - mouse.y) * smooth;

        INPUT in = {}; in.type = INPUT_MOUSE; in.mi.dwFlags = MOUSEEVENTF_MOVE;
        in.mi.dx = dx; in.mi.dy = dy; SendInput(1, &in, sizeof(in));
    }
}

// Overlay
LRESULT CALLBACK WndProc(HWND h, UINT m, WPARAM w, LPARAM l) { return DefWindowProc(h, m, w, l); }
void CreateOverlay() {
    WNDCLASSEX wc = {sizeof(wc), CS_CLASSDC, WndProc, 0, 0, GetModuleHandle(NULL), 0, 0, 0, 0, "OV", 0};
    RegisterClassEx(&wc);
    hOverlay = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_LAYERED, "OV", "Overlay", WS_POPUP, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, NULL, NULL, GetModuleHandle(NULL), NULL);
    SetLayeredWindowAttributes(hOverlay, 0, 255, LWA_ALPHA);
    ShowWindow(hOverlay, SW_SHOW);
}

void RenderLoop() {
    HDC hdc = GetDC(hOverlay);
    int w = rect.right - rect.left, h = rect.bottom - rect.top;
    HDC memDC = CreateCompatibleDC(hdc);
    HBITMAP bmp = CreateCompatibleBitmap(hdc, w, h);
    SelectObject(memDC, bmp);
    Graphics g(memDC);
    Pen red(Color(255,255,0,0), 2);

    while (enabled) {
        std::this_thread::sleep_for(std::chrono::milliseconds(16));
        GetWindowRect(hRoblox, &rect);
        SetWindowPos(hOverlay, HWND_TOPMOST, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, 0);

        g.Clear(Color(0,0,0,0));
        if (espOn) {
            std::lock_guard<std::mutex> lock(mtx);
            for (const auto& p : players) {
                Vector2 h, f;
                if (!W2S(p.head, h) || !W2S(p.feet, f)) continue;
                float bh = f.y - h.y, bw = bh / 2;
                g.DrawRectangle(&red, h.x - bw/2, h.y, bw, bh);
            }
        }
        POINT pt{}; SIZE sz{w, h};
        BLENDFUNCTION bf{AC_SRC_OVER, 0, 255, AC_SRC_ALPHA};
        UpdateLayeredWindow(hOverlay, hdc, NULL, &sz, memDC, &pt, 0, &bf, ULW_ALPHA);
    }
    DeleteObject(bmp); DeleteDC(memDC); ReleaseDC(hOverlay, hdc);
}

// Main
int main() {
    std::cout << "Roblox Offline Dev Tool - Press Enter to start\n";
    std::cin.get();

    while (!FindRoblox()) {
        std::cout << "Waiting for Roblox Studio (Play Solo)...\n";
        std::this_thread::sleep_for(std::chrono::seconds(5));
    }

    GetWindowRect(hRoblox, &rect);
    CreateOverlay();

    std::thread(MemoryLoop).detach();
    std::thread(HotkeyLoop).detach();
    std::thread(AimbotLoop).detach();
    std::thread(RenderLoop).join();

    CloseHandle(hProcess);
    return 0;
}
